...
export function renderRetailSnapshot({ retail, tacos, adSalesShare, adCpa, businessRows }) {
  const panel = document.getElementById("retailPanel");
  if (!panel) return;

  if (!retail || !businessRows || !businessRows.length) {
    panel.classList.add("hidden");
    return;
  }
  panel.classList.remove("hidden");

  document.getElementById("kpiRetailSales").textContent = formatMoney(retail.sales);
  document.getElementById("kpiRetailOrders").textContent = fmtInt(retail.orders);
  document.getElementById("kpiRetailUnits").textContent = fmtInt(retail.units);
  document.getElementById("kpiRetailSessions").textContent = fmtInt(retail.sessions);
  document.getElementById("kpiUnitSessionPct").textContent = formatPct(retail.unitSessionPct);
  document.getElementById("kpiTacos").textContent = formatPct(tacos);
  document.getElementById("kpiAdSalesShare").textContent = formatPct(adSalesShare);
  document.getElementById("kpiAdCpa").textContent = formatMoney(adCpa);
  ...
  for (const r of top) {
    ...
    tr.innerHTML = `
      ...
      <td>${formatMoney(r.sales || 0)}</td>
      <td>${fmtInt(r.sessions || 0)}</td>
      <td>${formatPct(r.sessionPct)}</td>
      <td>${fmtInt(r.units || 0)}</td>
      <td>${formatPct(r.unitSessionPct)}</td>
    `;
    tbody.appendChild(tr);
  }
}
