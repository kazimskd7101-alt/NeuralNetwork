import { formatMoney, formatNum, formatPct } from "./metrics.js";
...
export function renderIssuesTable(issues) {
  ...
  for (const it of issues) {
    ...
    tr.innerHTML = `
      <td><span class="badge ${it.type==="Waste"?"bad":(it.type==="Spike"?"warn":"warn")}">${it.type}</span></td>
    ...
    el.innerHTML = `
      <div class="action-top">
        <div>
          ...
          <div class="action-title" style="margin-top:10px">${escapeHtml(a.title)}</div>
...
export function renderDrillTable({ level, rows, onlyZeroSales }) {
  ...
  const filtered = onlyZeroSales
    ? rows.filter(r => r.zero_sales_spend_flag === true)
    : rows;
